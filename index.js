...
// ================= TIKTOK CHECK =================
// TikTok tidak punya API publik resmi.
// Ini validasi via halaman publik https://www.tiktok.com/@username.
// Kalau request diblok TikTok, validasi bisa gagal.
async function getTikTokUser(username) {
  ...
  if (!r.ok) {
    throw new Error(`TikTok check failed: ${r.status}`);
  }
  ...
  if (notFoundSignals.some((s) => lowered.includes(s))) return null;
...
function buildRegisterModal() {
  ...
  const tiktokUsername = new TextInputBuilder()
    ...
    .setPlaceholder("Contoh: budiAjadeh")
    .setStyle(TextInputStyle.Short)
    .setRequired(false)
    .setMaxLength(30);
...
function buildConfirmationEmbed(data) {
  ...
  if (data.tiktok?.avatarUrl) {
    embed.setImage(data.tiktok.avatarUrl);
  }
  ...
  try {
    ...
    if (i.isModalSubmit() && i.customId === "fs_register_modal_submit") {
      ...
      if (tiktokUsername && !/^[A-Za-z0-9._]{2,30}$/.test(tiktokUsername)) {
        return i.editReply({
          content:
            "Username TikTok tidak valid. Isi tanpa @ dan gunakan username TikTok yang benar.",
          components: buildRetryButton(),
        });
      }
      ...
      if (tiktokUsername) {
        try {
          tiktok = await getTikTokUser(tiktokUsername);
        } catch (e) {
          console.error("TikTok check error:", e);

          return i.editReply({
            content:
              `Gagal cek username TikTok \`${tiktokUsername}\`.\n` +
              `Pastikan username benar atau kosongkan jika tidak punya TikTok.`,
            components: buildRetryButton(),
          });
        }
        ...
        if (!tiktok) {
          return i.editReply({
            content:
              `Username TikTok \`${tiktokUsername}\` tidak ditemukan. Silakan isi ulang atau kosongkan.`,
            components: buildRetryButton(),
          });
        }
      }
